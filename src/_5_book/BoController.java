...
public class BoController implements Initializable {
			...
			public DateCell call(final DatePicker param) {
				return new DateCell() {
					...
					public void updateItem(LocalDate item, boolean empty) {
						...
						LocalDate endDay = LocalDate.now().plusDays(13);
	...
	public void bookButtonProc() {
		...
		if(boT1.isSelected()==false
			...
			if(boService.selectPtcount(id)==0) {
				...
				point = boSetBranch.getValue();
				workoutdayinsertservice(date,id,point);
				String time = "";
				if(boT1.isSelected()) {
					time = "T1";
					boService.bookPtTime(trainername,name, time, date,id,point);
	...
	public void workoutdayinsertservice(String date,String id, String point ) {
		point = boSetBranch.getValue();
		boService.workoutdayinsert(date, id, point);
	}
